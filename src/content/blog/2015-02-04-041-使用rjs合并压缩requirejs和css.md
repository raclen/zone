---
title: "使用rjs合并压缩requirejs和css"
description: "需要压缩的index.js 在项目目录下新建一个rjs_build文件夹,里面放上r.js 再写一个buildjs.js 进入rjs_build目录 node r.js -o buildjs  2)合并压缩css 新建一个main.css(名称自定义)，里面使用import 引入需要合并的css，从..."
pubDate: 2015-02-04T07:40:00.000Z
issueNumber: 41
issueUrl: https://github.com/raclen/zone/issues/41
...
```
在项目目录下新建一个rjs_build文件夹,里面放上r.js
再写一个buildjs.js
```javascript
({
    baseUrl: '../js/',
    out : 'index-build.js',//合并压缩后，输出JS文件地址和名称
    name: 'testRequire/index',//需要压缩的JS文件，相对于baseUrl
...
node r.js -o buildjs
 2)合并压缩css
新建一个main.css(名称自定义)，里面使用import 引入需要合并的css，从上到下的顺序，例如
@import url('reset.css');
@import url('se.popup.css');

在rjs_build文件夹中，新建一个buildcss.js
```javascript
node r.js -o buildcss.js
({
...
```
也可以不新建buildcss.js文件使用
node r.js -o cssIn=../css/main.css out=../css/main.min.css optimizeCss=standard
相关代码放在https://github.com/ralcen/jquery/tree/master/requirejs
2.dir输出
dir方式的压缩，会输出一个新的项目到指定的文件夹，适合一个项目一个文件夹的项目结构，如果很多项目并排放着，就很麻烦了，要写正则过滤，如果层级比较多，想到就头疼，但是dir可以把css和JS及I18文件一次压缩了，还是挺好的，这个我就不细说了，去年写过
相关代码放在https://github.com/ralcen/jsCode/tree/master/requireJs
## rjs参数说明
```javascript
/*
...
```
另外这里有份比较全的[R.JS配置](http://nomospace.com/posts/r.js-example.build.js.html)
jquery里面有段奇怪的写法
```
propFix: {
            tabindex: "tabIndex",
...
```
这里有for和class关键字，打包的时候会去掉引号，IE8以下报错
无论是gulp-uglify还是 r.js都是使用uglify压缩的
可以通过这个配置来解决
```
  uglify: {
